...
function titleCase(s) {
  ...
  if (s) {
    ...
    splitStr.forEach((element) => {
      retStr += element.charAt(0).toUpperCase() + element.slice(1) + " ";
    });
...
function transformObservations(observations) {
  return observations.map(function (observation) {
    ...
    var obj = {
      ...
      photoUrl: observation.taxon.default_photo.medium_url,
      wikipediaUrl: observation.taxon.wikipedia_url,
      isNative: String(observation.taxon.native) === "true",
      isIntroduced: String(observation.taxon.introduced) === "true",
      isEndangered: String(observation.taxon.endangered) === "true",
      isThreatened: String(observation.taxon.threatened) === "true",
    };
    return obj;
  });
}
...
function filterOnlyNative(observations) {
  ...
  const filtered = filterObservations(data.results);
  const transformed = transformObservations(filtered);
  return transformed.filter(function (observation) {
