 ...
 ******************************************************************************/
const observations = require('./data.js');
...
function observationSummary(data) {
    for (let i = 0; i < getResults(data).length; i++) {
        console.log(
            `#${getResults(data)[i].id} - ${getResults(data)[i].species_guess} (${
        getResults(data)[i].observed_on_details.date
      })`
        );
    }
}
...
function observationsByGeoPrivacy(data, geoPrivacy) {
    ...
    if (geoPrivacy !== null && typeof geoPrivacy !== 'string') {
        return retArr;
    }
    for (const element of getResults(data)) {
        if (geoPrivacy === null) {
            if (element.geoprivacy === null) {
                retArr.push(element);
            }
        } else if (geoPrivacy.toLowerCase() === element.geoprivacy) {
            retArr.push(element);
        }
    }
    return retArr;
...
function transformObservations(data) {
    let retArr = new Array();
    getResults(data).forEach((element) => {
        retArr.push(transformObservation(element));
    });
    return retArr;
}
...
function getObservationsById(data, ...ids) {
    ...
    let found;
    ids.forEach((i) => {
        found = getResults(data).find((element) => i === element.id);
        if (found) {
            retArr.push(found);
        }
    });
...
function getObservationsByPositionalAccuracy(data, options = {}) {
    let { min, max, equal } = options;
    let filter;
    if (options.equal) {
        filter = (x) => x.positional_accuracy === equal;
    } else if (options.min && options.max) {
        filter = (x) => x.positional_accuracy >= min && x.positional_accuracy <= max;
    } else if (options.min) {
        filter = (x) => x.positional_accuracy >= min;
    } else if (options.max) {
        filter = (x) => x.positional_accuracy <= max;
    } else {
        filter = (x) => x;
    }
...
function getTaxonPhotos(data) {
    ...
    getResults(data).forEach((element) => {
        if (element.taxon && element.taxon.default_photo) {
            let id = element.taxon.default_photo.id;
            let photo = {
                original: `https://static.inaturalist.org/photos/${id}/original.jpg`,
                square: `https://static.inaturalist.org/photos/${id}/square.jpg`,
                small: `https://static.inaturalist.org/photos/${id}/small.jpg`,
                medium: `https://static.inaturalist.org/photos/${id}/medium.jpg`,
                large: `https://static.inaturalist.org/photos/${id}/large.jpg`
            };
            retArr.push(photo);
        }
    });
...
function getUserStats(data) {
    let user_count = 0,
        total_obervations_count = 0,
        total_identifications_count = 0,
        total_species_count = 0;
    getResults(data).forEach((element) => {
        user_count++;
        total_obervations_count += element.user.observations_count;
        total_identifications_count += element.user.identifications_count;
        total_species_count += element.user.species_count;
    });
    let retObj = {
        count: user_count,
        totals: {
            observations: total_obervations_count,
            identifications: total_identifications_count,
            species: total_species_count
        },
        averages: {
            observations: total_obervations_count / user_count,
            identifications: total_identifications_count / user_count,
            species: total_species_count / user_count
        }
    };
    return retObj;
}
